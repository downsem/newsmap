  ...
  const addMarkers = useCallback((map, storyClusters) => {
    ...
    storyClusters.forEach((cluster) => {
      ...
      el.addEventListener('click', () => {
        setSelected(cluster);
        map.flyTo({ center: cluster.coordinates, zoom: Math.max(map.getZoom(), 9), speed: 0.7 });
      });
    ...
    return (
      <main className="token-warning">
        <section className="token-card">
          ...
          <p>Create a free Mapbox token, then add it to <strong>.env.local</strong>. This MVP already includes the map UI, story clusters, mock on-demand API, saved stories, and saved locations.</p>
    ...
    <main className="app-shell">
      ...
      <header className="topbar">
        <section className="brand-card">
          ...
          <p>Explore news by place. Zoom from national context to local story clusters, then tap a pin for a source-linked summary.</p>
